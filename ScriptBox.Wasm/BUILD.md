...
This document explains how to build the custom QuickJS WASM module (`scriptbox.wasm`) that includes the `scriptbox_wrapper.c` bridge to your C# host.

## Overview

The build process:
1. Downloads or uses local QuickJS source
2. Compiles QuickJS + scriptbox_wrapper.c to WASM (WebAssembly)
3. Outputs `scriptbox.wasm` which exports:
...
- `scriptbox.wasm` is created in the `wasm/` directory
...
To test the generated `scriptbox.wasm`:

```bash
# Using wasmtime CLI (if installed)
wasmtime quickjs-scriptbox.wasm
...
- The bootstrap automatically defines `globalThis.scriptbox` with methods that use `host_call`
