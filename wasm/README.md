...
- **`assistant_wrapper.c`**: The main C bridge code that:
  ...
  - Defines `globalThis.assistantApi` with JS bindings to `host_call`
...
This produces `assistant.wasm` (~200-300KB).
...
It copies `assistant.wasm` to the output directory.
...
└──────────────┬──────────────────────────┘
               ...
               ▼
┌─────────────────────────────────────────┐
│ assistant.wasm (QuickJS + wrapper)      │
│ - eval_js() parses & executes JS        │
│ - JS calls assistantApi.log/add         │
...
2. Add a new method to the JS bootstrap in `assistant_wrapper.c`
...
- JavaScript errors are printed (if enabled in assistant_wrapper.c)
...
To add a new method like `assistantApi.divide(a, b)`:
...
2. **Add C bootstrap** in `assistant_wrapper.c`:
   ```c
