...
public sealed class ScriptBoxBuilder
{
    ...
    public ScriptBoxBuilder UseAttributedApis(params Assembly[] assemblies)
    {
        Assembly[] targets;
        ...
        {
            targets = new[] { Assembly.GetCallingAssembly() };
        }

        var descriptors = AttributedSandboxApiRegistry.DiscoverApis(targets);
        ...
        ExposeHostApi(bootstrap, hostApi =>
        {
            AttributedSandboxApiRegistry.RegisterHandlers(descriptors, hostApi);
            return hostApi;
        });
